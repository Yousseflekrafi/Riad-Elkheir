...
if (isset($_GET['delete'])) {
    $stmt = $pdo->prepare("DELETE FROM administration_member WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_GET['edit'])) {
    $stmt = $pdo->prepare("INSERT INTO administration_member (full_name, email, phone, cin, job, date_join, date_left, salary) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([
        $_POST['full_name'],
        $_POST['email'],
        $_POST['phone'],
...
// --- Handle Update ---
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_GET['edit'])) {
    $stmt = $pdo->prepare("UPDATE administration_member SET 
        full_name=?, email=?, phone=?, cin=?, job=?, date_join=?, date_left=?, salary=? 
        WHERE id=?");
    $stmt->execute([
        $_POST['full_name'],
        $_POST['email'],
    ...
    if (!empty($search)) {
        $res = $pdo->prepare("SELECT * FROM administration_member 
                WHERE full_name LIKE :search OR email LIKE :search OR cin LIKE :search 
                ORDER BY id DESC");
        $res->execute([':search' => "%$search%"]);
    } else {
        $res = $pdo->query("SELECT * FROM administration_member ORDER BY id DESC");
    }
...
$edit_member = null;
if (isset($_GET['edit'])) {
    $stmt = $pdo->prepare("SELECT * FROM administration_member WHERE id = ?");
...
<head>
    ...
    <title>Administration Members - Riad Elkheir</title>
...
<nav class="navbar navbar-expand-lg navbar-light bg-white shadow-sm">
    ...
    <section class="formSection">
        ...
        <form method="POST">
            ...
            <?php if ($edit_member): ?>
            <a href="administration.php">Annuler</a>
            <?php endif; ?>
        </form>
        </div>
    </section>
    ...
    <section class="table-section">
        ...
        <div class="table-container">
            ...
            <thead>
                <tr>
                <th>ID</th><th>Nom et Prenom</th><th>Email</th><th>Telephone</th><th>CIN</th>
